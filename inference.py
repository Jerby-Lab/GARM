...
def evaluate(loader, model, uncertainty, device, ctrl=None):
    """
    ...
    results = {}
    logvar = []
    has_pred = hasattr(model, 'predict') and callable(model.predict)
        ...
        with torch.no_grad():
            if uncertainty:
                ...
                if has_pred:
                    ...
                    p,_,_ = model.predict(pred_mean-ctrl, pred_var_row-ctrl, pred_pear_row-ctrl, pred_var_col-ctrl, pred_pear_col-ctrl)
                    p = p + ctrl
                else:
                    p = model(batch)
            t = batch.y
            ...
            for itr, de_idx in enumerate(de_idx):
...
def compute_metrics(results):
    ...
    for pert in np.unique(results['pert_cat']):
        ...
        if results['pred_de'].shape[1] > 1 and pert != 'ctrl' and pert != 'non-targeting':
